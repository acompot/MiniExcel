...
std::string Position::ToString() const {
  if (IsValid()) {
    return detail::NumberToLetters(col) + std::to_string(row + 1);
  }
  return {};
}
...
Position Position::FromString(std::string_view str) {
  size_t num_start;
  if (detail::IsCorrect(str, num_start)) {
    int col = detail::LettersToNumber(str.substr(0, num_start));
...
namespace detail {
  ...
  int LettersToNumber(std::string_view letters) {
    ...
    for (size_t i = 0; i < len; ++i) {
      ...
      result += static_cast<int>(c - 'A' + 1) * std::pow(detail::LETTERS, i);
    }
    return result - 1;
}
  ...
  bool IsCorrect(std::string_view str, size_t &first_digit_pos) {
    ...
    for (auto c : str) {
     ...
     ++num_digits;
      ...
      return num_digits > 0 && num_digits <= detail::MAX_DIGITS_COUNT && num_letters > 0 && num_letters <= detail::MAX_LETTERS_COUNT;
  }
